...
Le fichier de démarrage est \textbf{autonome}~: toutes les fonctions sont implémentées directement dans le code, sans dépendance externe. Il contient un dictionnaire embarqué~:
...
\begin{tabular}{lll}
  ...
  \midrule
  \texttt{WORDS-XS-FR} & 100 mots  & Dictionnaire minimal (mots de 5 lettres), embarqué \\
  \texttt{WORDS-S-FR}  & 5~000 mots  & À charger depuis \texttt{dictionaries/WORDS-S-FR.txt} \\
  \texttt{WORDS-M-FR}  & 13~000 mots & À charger depuis \texttt{dictionaries/WORDS-M-FR.txt} \\
  \texttt{WORDS-L-FR}  & 40~000 mots & À charger depuis \texttt{dictionaries/WORDS-L-FR.txt} \\
  \bottomrule
\end{tabular}
\end{center}

L'alphabet utilisé (\texttt{ALPHABET-FR-SIMPLE}) inclut les 26 lettres de base + les 6 accents les plus courants (é, è, ê, à, ù, ç). Une version complète (\texttt{ALPHABET-FR}) est également disponible avec æ, œ, ë, ï, ô, ö, û, ü.
...
\begin{codebox}
  ...
  \begin{tabularx}{\textwidth}{lX}
    \texttt{subs(mot, alpha)}     & Remplace chaque lettre une par une par toutes les lettres de l'alphabet. Ex.~: \texttt{subs("chein", ALPHABET-FR-SIMPLE)} génère «~ahien, bhien, \ldots, chien, \ldots{}~» \\
    \texttt{swaps(mot)}    & Échange chaque paire de lettres adjacentes. Ex.~: \texttt{swaps("chein")} génère «~hcein, cehin, chien, chein~» \\
    \texttt{insertions(mot, alpha)} & Insère chaque lettre de l'alphabet à chaque position. Ex.~: \texttt{insertions("aboi", ALPHABET-FR-SIMPLE)} génère «~aaboi, baboi, \ldots, aboie, \ldots{}~» \\
    \texttt{deletions(mot)}  & Supprime chaque lettre une par une. Ex.~: \texttt{deletions("vossinage")} génère «~ossinage, \ldots, voisinage, \ldots{}~» \\
    \texttt{only-real(liste, dico)} & Filtre une liste pour ne garder que les mots présents dans le dictionnaire. \\
    \texttt{alt-words(mot, dico, n, alpha)} & Combine toutes les fonctions et trouve les mots du dictionnaire à distance $\leq$~\texttt{n}. \\
  \end{tabularx}
\end{codebox}
...
\begin{activitybox}
  ...
  \begin{itemize}
    \item Dans la fenêtre d'interactions à droite, évaluez \texttt{alt-words("plation", WORDS-L-FR, 4, ALPHABET-FR-SIMPLE)}.
    \begin{itemize}
...
\begin{codebox}
  Évaluez \texttt{resultats = alt-words("plation", WORDS-L-FR, 4, ALPHABET-FR-SIMPLE)}.
...
\begin{activitybox}
  \begin{itemize}
    \item Ajoutez \texttt{resultats = alt-words("plation", WORDS-L-FR, 4, ALPHABET-FR-SIMPLE)} comme \textbf{dernière ligne de la zone de définitions}, et cliquez sur «~Run~». Évaluez \texttt{resultats} dans la zone d'interactions.
...
\begin{tabular}{ll}
  ...
  \midrule
  \texttt{Correcteur-Orthographique-Starter.arr} & Fichier Pyret autonome (tout réimplémenté) \\
...
\vspace{1em}
\textbf{À noter~:} contrairement à la version anglaise originale qui charge une bibliothèque distante, le fichier \texttt{.arr} est entièrement autonome. Toutes les fonctions (\texttt{subs}, \texttt{swaps}, \texttt{insertions}, \texttt{deletions}, \texttt{only-real}, \texttt{alt-words}) sont réimplémentées en Pyret pur. L'alphabet inclut les caractères accentués français, et le dictionnaire \texttt{WORDS-XS-FR} (100 mots) est embarqué pour des démonstrations instantanées. Les grands dictionnaires sont à charger depuis les fichiers \texttt{.txt} du dossier \texttt{dictionaries/}.

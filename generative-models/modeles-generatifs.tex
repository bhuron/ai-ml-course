...
\title{%
  ...
  {\normalsize Adapté au contexte francophone — corpus de la chanson «~Il était une dame qui avalait une mouche~»}\\[0.5em]
...
Pour notre corpus, nous utiliserons la chanson enfantine française \textbf{«~Il était une dame qui avalait une mouche~»} — l'équivalent français de la chanson anglaise \emph{There Was an Old Lady Who Swallowed a Fly} — qui raconte l'histoire absurde d'une dame qui avale une mouche, et la série d'événements malheureux qui s'ensuit.
...
\begin{activitybox}
  ...
  \begin{tabularx}{\textwidth}{c c X}
    ...
    \midrule
    1 mot  & 8 & (Il) (était) (une) (dame) (qui) (avalait) (une) (mouche) \\
    2 mots & 7 & (Il était) (était une) (une dame) (dame qui) (qui avalait) (avalait une) (une mouche) \\
    3 mots & 6 & (Il était une) (était une dame) (une dame qui) (dame qui avalait) (qui avalait une) (avalait une mouche) \\
    \bottomrule
  \end{tabularx}
  \end{center}
\end{activitybox}
...
\begin{activitybox}
  \begin{itemize}
    ...
    \item Évaluez \texttt{generate-ngrams-fr(corpus, 1)}.
  \end{itemize}
\end{activitybox}

\subsubsection{Approfondissons~: la probabilité conditionnelle}

La phrase «~il était une dame qui avalait une…~» est répétée dans notre corpus~! Concentrons-nous sur un unigramme de cette phrase~: «~il~».

\begin{thinkbox}
  \begin{itemize}
    \item \textbf{En vous référant au corpus (les paroles \emph{et le titre})~: combien de fois le mot «~il~» apparaît-il dans la chanson~?}
    \begin{itemize}
      \item 5.
    \end{itemize}
    \item \textbf{Dans ce corpus, combien de fois le mot «~il~» a-t-il été suivi du mot «~était~»~?}
    \begin{itemize}
      \item 5.
    \end{itemize}
    \item \textbf{Quelle est la probabilité que le mot «~il~» soit suivi du mot «~était~»~?}
    \begin{itemize}
      \item 5/5, soit 100~\%.
    \end{itemize}
  \end{itemize}
...
Dans l'exemple que vous venez de traiter, vous avez calculé la \textbf{probabilité conditionnelle} que «~était~» apparaisse après l'unigramme «~il~» en divisant le nombre de fois où \textbf{«~il~» est suivi de «~était~»} (cinq fois) par le nombre de fois où nous voyons \textbf{«~il~» suivi de n'importe quoi} (cinq fois).

Cette fraction représente la probabilité que «~était~» vienne après «~il~»~:

\[
p(\text{était} \mid \text{il}) = \frac{\text{nombre de (il était)}}{\text{nombre de (il…)}} = \frac{5}{5}
\]
...
\begin{activitybox}
  \begin{itemize}
    ...
    \item \textbf{Dans la Zone de Définitions}, construisez votre modèle de langue en évaluant \texttt{m = build-lang-model-fr(corpus)}, puis cliquez sur «~Run~».
    \item Évaluez \texttt{m} pour voir à quoi il ressemble~!
    \item Pour trouver la probabilité qu'un mot suive un autre, évaluez~:
    \begin{lstlisting}
next-word-probability-fr(m, "avalait une", "mouche")
    \end{lstlisting}
  \end{itemize}
\end{activitybox}
...
\begin{keypointbox}
  ...
  Notre fichier de démarrage fournit des versions \texttt{-fr} autonomes de toutes ces fonctions (\texttt{massage-string-fr}, \texttt{generate-ngrams-fr}, \texttt{build-lang-model-fr}, \texttt{completions-fr}, \texttt{next-word-probability-fr}, \texttt{choose-completion-fr}, \texttt{generate-from-fr}). Les différences avec l'original~:
  \begin{enumerate}
...
\begin{thinkbox}
  ...
  \begin{itemize}
    \item il y avait \emph{trois} complétions possibles pour l'unigramme «~avala~»~: \texttt{l}, \texttt{un}, \texttt{le}.
...
\begin{keypointbox}
  ...
  \contrat{\# completions-fr :: (Model model, String debut) -> Table\\
\# choose-completion-fr :: (Model model, String debut, Number n) -> String}

  \begin{itemize}
    \item Nous pouvons utiliser ces contrats pour trouver toutes les complétions possibles d'une chaîne de départ~:
    \begin{lstlisting}
completions-fr(m, "avalait une")
    \end{lstlisting}
    \item Ou pour choisir les $n$ prochains mots au hasard~:
    \begin{lstlisting}
choose-completion-fr(m, "avalait une", 1)
choose-completion-fr(m, "avalait une", 2)
    \end{lstlisting}
  \end{itemize}
\end{keypointbox}
...
\begin{activitybox}
  ...
  \textbf{Réponse attendue~:} \texttt{elle avala l araignée}

  \begin{itemize}
    \item \texttt{elle} $\to$ \texttt{avala} (7 occurrences, le plus fréquent) $\to$ \texttt{l} (5) $\to$ \texttt{araignée} (5). Il n'y a jamais d'égalité~: chaque étape a un mot clairement le plus probable.
  \end{itemize}
\end{activitybox}
...
\begin{activitybox}
  ...
  Voici une liste des unigrammes les plus courants du corpus~: \texttt{"elle"}~: 15~fois, \texttt{"une"}~: 12~fois, \texttt{"avalait"}~: 9~fois, \texttt{"mouche"}~: 9~fois.
...
\begin{activitybox}
  \begin{itemize}
    ...
    \begin{itemize}
      \item La probabilité que «~zèbre~» soit suivi de chaque mot est la même~: zéro. Peut-être que \texttt{choose-completion-fr} choisirait au hasard~?
    \end{itemize}
  \end{itemize}
\end{activitybox}

\begin{instructornotes}
  Quand vous avez terminé, discutez~: que se passerait-il si nous continuions à évaluer \texttt{choose-completion-fr}, encore et encore~?

  \begin{itemize}
    \item Il continuerait à choisir des mots~!
  \end{itemize}

  Si vous êtes curieux·se de voir ce qui se passerait si nous continuions, évaluez \texttt{generate-from-fr(m, "il était")}~!
...
Les nombres ci-dessous ont été calculés sur le corpus français \emph{après normalisation} (minuscules, apostrophes transformées en espaces, ponctuation supprimée — exactement ce que fait \texttt{massage-string-fr}). Ils correspondent à ce que Pyret calcule.
...
\begin{tabularx}{\textwidth}{l X}
  ...
  \midrule
  Mots au total & 178 \\
  Unigrammes distincts & 40 \\
  Bigrammes distincts & 70 \\
  Trigrammes distincts & 82 \\
  Unigramme le plus courant & \texttt{elle} (15~fois) \\
  2\textsuperscript{e} unigramme le plus courant & \texttt{une} (12~fois) \\
  3\textsuperscript{e} unigramme le plus courant & \texttt{avalait} et \texttt{mouche} (9~fois chacun) \\
  \texttt{il} $\to$ \texttt{était} & 5/5 = 100\,\% \\
  \texttt{elle} $\to$ \texttt{avala} & 7 occurrences (vs.\ \texttt{avalait}~4, \texttt{va}~4) \\
  \texttt{avala} $\to$ \texttt{l} & 5 occurrences (vs.\ \texttt{un}~1, \texttt{le}~1) \\
  \texttt{l} $\to$ \texttt{araignée} & 5 occurrences (vs.\ \texttt{oiseau}~3) \\
  \texttt{une} $\to$ \texttt{mouche} & 6 occurrences (vs.\ \texttt{dame}~5, \texttt{araignée}~1) \\
...
La chanson anglaise \emph{There Was an Old Lady Who Swallowed a Fly} et sa version française \emph{Il était une dame qui avalait une mouche} ont une structure identique~:
...
\begin{tabularx}{\textwidth}{X X}
  ...
  \midrule
  There was an old lady who swallowed a fly & Il était une dame qui avalait une mouche \\
  I don't know why she swallowed a fly -- perhaps she'll die~! & Je ne sais pas pourquoi elle avalait une mouche -- peut-être qu'elle va mourir~! \\
...
\begin{itemize}
  ...
  \item La chanson française ajoute un défi intéressant pour le modèle de langue~: les \textbf{articles élidés}. En anglais, \texttt{the} est toujours un mot séparé. En français, \texttt{l'} est collé au nom suivant (\texttt{l'araignée}). Notre \texttt{massage-string-fr} transforme l'apostrophe en espace, ce qui permet de traiter \texttt{l} comme un mot à part entière — exactement comme \texttt{the} en anglais.
...
\begin{tabularx}{\textwidth}{l X}
  ...
  \midrule
  \texttt{Modeles\allowbreak-Generatifs\allowbreak-Starter.arr} & Fichier de démarrage Pyret~: charge \texttt{ai-library.arr} depuis Bootstrap, définit le corpus français et les fonctions \texttt{-fr} autonomes. \\

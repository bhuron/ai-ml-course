...
fun is-non-punct(c :: String) -> Boolean block:
  # Les espaces et les lettres (y compris accentuées françaises) sont conservés.
  if (c == ' ') or (c == '\n'): true
  else:
    # Lettres de base a-z / A-Z
    ...
    is-ascii-letter = ((c-cp >= lower-case-a-cp) and (c-cp <= lower-case-z-cp)) or
      ((c-cp >= upper-case-a-cp) and (c-cp <= upper-case-z-cp))
    if is-ascii-letter: true
    else:
      # Lettres accentuées françaises (minuscules et majuscules) : à â é è ê ë î ï ô ö ù û ü ç æ œ
      accents = [list:
...
fun massage-string(w :: String) -> String block:
  # Mise en minuscules, retours à la ligne -> espaces, apostrophes -> espaces
  # (ainsi "l'araignée" devient "l araignée", et l'article élidé "l'" est
  # traité comme un mot séparé), puis suppression de la ponctuation en
  # conservant les lettres accentuées françaises.
  lowercase-chars = string-explode(string-to-lower(w)).map(replace-newlines)
  no-apostrophes = lowercase-chars.map({(c): if c == "'": " " else: c end})
  fold({(a, b): a + b }, '', no-apostrophes.filter(is-non-punct))
end

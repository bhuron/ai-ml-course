...
\title{%
  ...
  {\normalsize Adapté au contexte francophone — textes, stop words et normalisation en français}\\[0.5em]
...
\begin{thinkbox}
  \begin{itemize}
    ...
    \begin{itemize}
      \item le titre, la phrase d'introduction, un mot qui revient souvent…
...
\begin{thinkbox}
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{itemize}
        \item Ponctuation~: \texttt{blaireau,} et \texttt{blaireau.} ne sont pas deux mots différents, mais le système les compte séparément à cause de la virgule et du point.
        \item Majuscules~: \texttt{Le} et \texttt{le} ne sont pas deux mots différents, mais le système les distingue à cause de la majuscule.
      \end{itemize}
    \end{itemize}
...
\begin{keypointbox}
  Les ingénieur·es en apprentissage automatique et les data scientistes se soucient beaucoup de normaliser leurs données afin de couper le bruit non pertinent et de se concentrer sur ce qui compte. Nous avons fourni trois fonctions de normalisation pour que vous expérimentiez~:

  \contrat{\# lowercase :: String -> String\\
\# remove-punct-fr :: String -> String\\
...
\begin{thinkbox}
  ...
  \begin{itemize}
    ...
    \item La fonction \texttt{remove\allowbreak-punct\allowbreak-fr} remplace les apostrophes par des espaces (ainsi \texttt{l'éléphant} devient \texttt{l eléphant}), ce qui permet de séparer les articles élidés.
    \item Nous n'avons pas de fonction pour supprimer les pluriels.
  \end{itemize}
\end{thinkbox}

\begin{keypointbox}
  \textbf{Adaptation française des fonctions de normalisation}

  Dans l'original anglais, la bibliothèque \texttt{ai-library.arr} de Bootstrap fournit une liste de \emph{stop words} anglais («~the~», «~and~», «~a~», etc.) et des fonctions de normalisation adaptées à l'anglais. Notre fichier de démarrage français \textbf{redéfinit} trois éléments~:
  \begin{enumerate}
    \item \texttt{stop-words} — remplacé par une liste de mots vides français («~le~», «~la~», «~les~», «~un~», «~et~», «~de~», «~est~», etc.).
    \item\texttt{remove-punct-fr} — nouvelle fonction qui \emph{préserve les accents français} (é, è, ê, à, ç, ù, etc.). La fonction originale de la bibliothèque supprimait tous les caractères en dehors de \texttt{a-z}, ce qui aurait détruit les accents~!
    \item \texttt{remove-stop-words-fr} et \texttt{normalize\allowbreak-text\allowbreak-table-fr} — utilisent la liste française et la nouvelle fonction de ponctuation.
  \end{enumerate}
  C'est la seule modification du code nécessaire pour adapter cette leçon au français.
\end{keypointbox}
...
\begin{keypointbox}
  En ajoutant l'apprentissage supervisé à notre boîte à outils, nous pouvons commencer à effectuer ce que les ingénieur·es ML appellent l'\textbf{analyse de sentiment} sur le \emph{ton} ou le \emph{sujet} de nos courriels et SMS~!
  \begin{itemize}
...
Le fichier de démarrage \texttt{Langage\allowbreak-Naturel\allowbreak-Starter.arr} redéfinit la liste des stop words avec une liste française standard, et fournit une version française de toutes les fonctions de normalisation qui préservent les accents. Voici quelques-uns des mots les plus courants qui sont supprimés lors de la normalisation~:
...
\begin{tabularx}{\textwidth}{l X}
  ...
  \midrule
  \texttt{Langage\allowbreak-Naturel\allowbreak-Starter.arr} & Fichier de démarrage Pyret~: charge \texttt{ai-library.arr} depuis Bootstrap. \textbf{Redéfinit} \texttt{stop-words} avec une liste française, ajoute \texttt{remove\allowbreak-punct\allowbreak-fr} (préserve les accents), et \texttt{remove-stop-words-fr} / \texttt{normalize\allowbreak-text\allowbreak-table-fr}. Textes d'animaux traduits en français. \\
...
\vspace{1em}
\textbf{À noter~:} contrairement à la leçon originale anglaise où les stop words et les fonctions de normalisation sont définis dans la bibliothèque Bootstrap, notre adaptation française redéfinit \texttt{stop-words} (avec \texttt{shadow}) et ajoute \texttt{remove\allowbreak-punct\allowbreak-fr}, \texttt{remove\allowbreak-stop\allowbreak-words-fr} et \texttt{normalize\allowbreak-text\allowbreak-table-fr} directement dans le fichier de démarrage. La fonction \texttt{remove\allowbreak-punct\allowbreak-fr} est \emph{indispensable} pour le français~: la fonction originale de la bibliothèque supprime tous les caractères en dehors de \texttt{a-z}/\texttt{A-Z}, ce qui détruit les accents (é, è, ê, à, ç, etc.). Notre version préserve les accents et remplace les apostrophes par des espaces. Toutes les autres fonctions de la bibliothèque fonctionnent inchangées.
